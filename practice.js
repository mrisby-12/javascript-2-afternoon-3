...
function uniq(array, callback) {
  let modArray = Array.from(new Set(array))
  callback(modArray);
};
...
function each(arrayNames, callback) {
  for (var i = 0; i < arrayNames.length; i++) {
    callback(arrayNames[i], i)
  }
};
...
function getUserById(usersObject, id, callback) {
  for (var i = 0; i < usersObject.length; i++) {
    if (id === usersObject[i].id) {
      callback(usersObject[i])
    }
  }
}
